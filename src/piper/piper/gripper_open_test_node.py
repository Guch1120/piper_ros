...
class OpenGripper(Node):
    def __init__(self):
        super().__init__('open_gripper_node')
        # MoveItの標準アクションサーバー 'move_group' に接続
        self._action_client = ActionClient(self, MoveGroup, 'move_action')
        self.get_logger().info('OpenGripper node initialized.')
    ...
    def send_goal(self):
        ...
        # ジョイント制約の作成 (SRDFのopen状態: joint7 = 0.035)
        constraints = Constraints()
        jc = JointConstraint()
        jc.joint_name = 'joint7'
        jc.position = 0.098  # Open position
    ...
    def get_result_callback(self, future):
        ...
        # error_code.val == 1 はSUCCESSを意味します
