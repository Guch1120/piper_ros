...
class MoveItTfClientState(EventState):
    '''
    TFフレームをターゲットとしてMoveIt(IK)で移動するState。
    ...
    ># target_frame     string  目標とするTFフレーム名 (例: 'interactive_set')
    ...
    def __init__(self, group_name='arm', end_effector_link='link6', reference_frame='base_link', 
                 pos_tolerance=0.01, orient_tolerance=0.01, action_topic='move_action'):
        super(MoveItTfClientState, self).__init__(outcomes=['reached', 'failed'],
    ...
    def on_enter(self, userdata):
        ...
        try:
            t = self._tf_listener.listener().lookup_transform(
                self._ref_frame, userdata.target_frame, rclpy.time.Time())
        except Exception as e:
            Logger.logerr(f'Could not get transform {self._ref_frame} -> {userdata.target_frame}: {e}')
            self._error = True
            return
        ...
        Logger.loginfo(f'Sending goal to TF: {userdata.target_frame}')
