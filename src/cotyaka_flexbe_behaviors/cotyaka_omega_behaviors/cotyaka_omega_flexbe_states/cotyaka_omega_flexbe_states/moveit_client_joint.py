...
class MoveItClientJoint(EventState):
    ...
    def on_enter(self, userdata):
        ...
        # 入力チェック
        if len(userdata.target_joints) != len(self._joint_names):
            Logger.logerr(f'Length mismatch: target_joints({len(userdata.target_joints)}) vs joint_names({len(self._joint_names)})')
            self._error = True
            return
        ...
        for i, name in enumerate(self._joint_names):
            jc = JointConstraint()
            jc.joint_name = name
            jc.position = float(userdata.target_joints[i])
        ...
        try:
            ...
            Logger.loginfo('Sent MoveGroup goal.')
