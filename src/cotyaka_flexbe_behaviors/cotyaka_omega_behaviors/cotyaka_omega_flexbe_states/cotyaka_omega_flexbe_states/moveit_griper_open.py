...
class PiperMoveItOpenState(EventState):
    '''
    MoveItを使用してPiperのグリッパを指定した幅まで開くステート。

    -- target_value  float   開く幅 [m] (default: 0.098)
                             ※対象コードの joint7=0.098 をデフォルトに設定
    ...
    def __init__(self, target_value=0.098, joint_name='joint7', group_name='gripper', tolerance=0.01, action_topic='move_action'):
        # FlexBEのエディタで設定可能なパラメータを定義
        super(PiperMoveItOpenState, self).__init__(outcomes=['reached', 'failed'])
        ...
        # ProxyActionClientのノード設定（FlexBEの仕様上の安全策）
        ...
        # Action Clientの作成（辞書形式でトピックと型を指定）
    ...
    def on_enter(self, userdata):
        ...
        goal.request.allowed_planning_time = 2.0  # 対象コードより
        ...
        try:
            ...
            Logger.loginfo(f'MoveIt: Opening gripper ({self._joint_name}) to {self._target_value}...')
    ...
    def execute(self, userdata):
        ...
        if self._client.has_result(self._topic):
            ...
            if result.error_code.val == 1:
                Logger.loginfo('Gripper OPENED successfully!')
                return 'reached'
            else:
    ...
    def on_exit(self, userdata):
        # ステートを抜ける際（Preempt時など）に実行中のゴールをキャンセルしてエラーを防ぐ
